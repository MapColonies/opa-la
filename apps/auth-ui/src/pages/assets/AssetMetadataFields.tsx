...
/** A non-breaking space keeps the hint's line reserved when there is no hint to put in it. */
const RESERVED_LINE = ' ';
...
export const AssetMetadataFields = ({ draft, onChange, uriError, disabled = false }: AssetMetadataFieldsProps) => {
  const targetsNothing = draft.environment.length === 0;
  ...
  return (
    <>
      <div className="flex flex-wrap items-end gap-6">
        ...
        <fieldset className="space-y-1" aria-describedby="asset-environments-hint">
      ...
      {/* Targeting nothing is permitted, and a valid work-in-progress state, but worth saying out
          loud. The line sits below the controls and is always here, so ticking a box neither
          moves the controls nor resizes the row. */}
      <p id="asset-environments-hint" className="text-xs text-muted-foreground" aria-hidden={!targetsNothing}>
        {targetsNothing ? 'No environments selected — this asset reaches no bundle.' : RESERVED_LINE}
      </p>
